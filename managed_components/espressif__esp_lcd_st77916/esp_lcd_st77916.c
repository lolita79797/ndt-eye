...
{
    uint8_t temp[16];
    if (param && param_size > 0 && param_size <= 16) {
        memcpy(temp, param, param_size);
        param = temp;
    }
    ...
    return esp_lcd_panel_io_tx_param(st77916->io, lcd_cmd, param, param_size);
}
...
static esp_err_t tx_color(st77916_panel_t *st77916, esp_lcd_panel_io_handle_t io, int lcd_cmd, const void *param, size_t param_size)
{
    ...
    return esp_lcd_panel_io_tx_color(st77916->io, lcd_cmd, param, param_size);
}
